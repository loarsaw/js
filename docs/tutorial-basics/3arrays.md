...
🔄 Unlike variables, 📦 an array can hold multiple values. Each value in an array has an 📌 index, and each index points to a 🧠 memory address. You can access each value using their indexes. The array's indexes start from 🕰️ zero, and the last element's index is one less than the array's 📏 length.

📚 An array is a collection of various data types that are 🧩 ordered and 🔄 changeable. It can store duplicate elements and diverse data types. An array can be 📭 empty or contain 🦄 different data type values.
